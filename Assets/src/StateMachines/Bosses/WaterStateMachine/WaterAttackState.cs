...
public class WaterAttackState : WaterBaseState
{
    ...
    public WaterAttackState(WaterStateMachine waterstateMachine, int attackIdx) : base(waterstateMachine)
    {
        
        this.attackDir = waterstateMachine.BossManager.FacingLeft? -1f:1f;
        this._attackIdx = attackIdx;
        //we want to stop the current move input
        //waterstateMachine.BossManager.RB.velocity = Vector2.zero;
       
        attack = waterstateMachine.BossManager.Attacks[attackIdx];
    ...
    public override void Enter()
    {
        ...
        waterstateMachine.BossManager.Animator.CrossFadeInFixedTime(attack.AnimationName, attack.TransitionDuration);
        waterstateMachine.BossManager.RB.velocity = Vector2.zero;
    }
    ...
    public override void Tick(float deltaTime)
    {
        ...
        impact = Vector2.SmoothDamp(impact, Vector3.zero, ref dampingVelocity,waterstateMachine.BossManager.Drag);
    }
    ...
    private void TryNextCombo(float normalizedTime)
    {
        ...
        waterstateMachine.SwitchState(
            new WaterAttackState(
                waterstateMachine,
     ...
     private void TryApplyForce()
    {
     ...
     private void AddForce(float force){
        impact = new Vector2(force,waterstateMachine.BossManager.RB.velocity.y);
    }
     ...
     private void setDamageWeapon(){
        ...
        if(waterstateMachine.SwordEnemies.TryGetComponent<SwordEnemies>(out SwordEnemies sword)){
